...
// INICIALIZA A VARIAVEL PARA EVITAR ERROR
$clientes = [];

// SE O FORMULARIO FOR ENVIADO, BUSCA O USUARIO POR ID OU NOME
if($_SERVER["REQUEST_METHOD"] == "POST" && !empty($_POST['busca'])) {
   $busca = trim($_POST['busca']);
    //VERFICA SE É UM NUMERO(ID) OU UM NOME
    if (is_numeric($busca)) {
        ...
        <?php foreach($clientes as $cliente): ?>
            <tr>
                ...
                <td>
                    ...
                    <a href="excluir_cliente.php?id=<?=htmlspecialchars($cliente['id_cliente']) ?>"onclick="return confirm('Tem certeza que deseja excluir esse cliente?')">Excluir</a>
            </tr>
        <?php endforeach; ?>
        </table>
